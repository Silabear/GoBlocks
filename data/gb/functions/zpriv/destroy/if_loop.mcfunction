...
# segment existing?
execute if entity @e[type=minecraft:marker,tag=gb.if_segment,limit=1,distance=..0.1] run scoreboard players set .destroyif gb.misc 1
execute unless entity @e[type=minecraft:marker,tag=gb.if_segment,limit=1,distance=..0.1] run scoreboard players set .destroyif gb.misc 0
