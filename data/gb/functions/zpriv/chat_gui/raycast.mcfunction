...
# hit
execute as @e[type=minecraft:marker,tag=gb.block,limit=1,sort=nearest,distance=..1] at @s run function gb:zpriv/chat_gui/has_hit
...
execute unless entity @e[type=minecraft:marker,tag=gb.block,limit=1,sort=nearest,distance=..1] run execute if score @s gb.raycast matches 1.. run execute positioned ^ ^ ^0.5 run function gb:zpriv/chat_gui/raycast